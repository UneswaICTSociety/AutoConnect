...
class ProxyTester:
    ...
    def is_proxy_configured():
        try:
            if get_os_type() == "Windows":
                import winreg
                # check wininet user-level proxy settings
                # doesnt cover winhttp system services proxy
...
class InternetTester:
    @staticmethod
    def test_internet_access(use_proxy=True):
