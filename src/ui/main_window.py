...
class StatusBar(ctk.CTkFrame):
    ...
    def update_status(self, wifi_connected, proxy_configured):
        if wifi_connected:
...
class CredentialsFrame(ctk.CTkFrame):
    ...
    def __init__(self, parent, translator: Optional[Callable[[str], str]] = None):
        super().__init__(parent)
        # if no translator provided, just return the key as-is
        ...
        # when user presses enter in student id, move to birthday field
    ...
    def _handle_birthday_enter(self, event):
        ...
        # validate then trigger setup if ok
        valid, message = self.validate_credentials()
        if valid:
            # reach up to main window to trigger action
            # ugly but works for small ui
    ...
    def validate_credentials(self):
        ...
        # delegate to network layer for validation
        try:
...
class ActionButtonsFrame(ctk.CTkFrame):
    ...
    def __init__(self, parent, callbacks, translator: Optional[Callable[[str], str]] = None):
        super().__init__(parent)
        self.callbacks = callbacks
        # fallback translator if none provided
        self._t = translator or (lambda k: k)
...
class UNESWAWiFiApp:
    ...
    def _setup_keyboard_shortcuts(self):
        # lambda converts keyboard events to method calls
...
ICT Society - University of Eswatini"""
    ...
    def _run_operation(self, operation, operation_name):
        # run operation in background so ui doesnt freeze
        ...
        def worker():
            ...
            try:
                ...
                operation()  # actual work happens here
            except Exception as e:
    ...
    def _do_complete_setup(self):
        # main button that does everything

        def setup():
            valid, msg = self.credentials_frame.validate_credentials()
            if not valid:
                self._log(f"{msg}")
                return
            ...
            if results["overall"]["success"]:
                ...
                if results["wifi"]["success"]:
                    summary += "✓ WiFi connected\n"
                if results["registration"]["success"]:
                    if results["registration"].get("already_registered"):
        ...
        def device_reg():
            ...
            if result.success:
                ...
                if result.already_registered:
                    msgbox.showinfo(
                        "Already Registered",
                        "Your device is already registered on the network.\n\n"
                        "You can connect to WiFi without restarting."
                    )
                else:
                    msgbox.showinfo(
                        "Registration Complete",
                        "Device registered successfully!\n\n"
                        "Please restart your device to complete the setup."
                    )
            else:
                self._log(f"Registration failed: {result.message}")
                msgbox.showerror("Registration Failed", result.message)
    ...
    def _start_monitoring(self):
        # check wifi/proxy status every 30 seconds in background
        ...
        def monitor():
            ...
            while self.monitor_running:
                try:
                    self._update_connection_status()
                    time.sleep(MONITOR_INTERVAL)  # wait 30 sec between checks
                except Exception:
